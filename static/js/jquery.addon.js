...
jQuery.setJS = function(list, callback) {
    ...
    if(list != null) {
        global.batchLoad_idx = 0;
        global.batchLoad_list = list;
        global.batchLoad_callback = callback;
    }
    if(global.batchLoad_list == null) return;
    if(global.batchLoad_idx >= global.batchLoad_list.length) {
        if(typeof(global.batchLoad_callback)=='function') global.batchLoad_callback();
        global.batchLoad_idx = 0;
        global.batchLoad_list = null;
        global.batchLoad_callback = null;
        return;
    }
    $.getScript(global.batchLoad_list[global.batchLoad_idx], function(){
        global.batchLoad_idx += 1;
        jQuery.setJS();
    });
}
